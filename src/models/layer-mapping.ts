  ...
  layers: [
    {
      id: "primitives-core",
      order: 0,
      collectionName: "00_Primitives_Core",
      displayName: "Primitives Core",
      description: "Core palettes: primary, secondary, sparkle, neutral",
      enabled: true,
      isAutoGenerated: true,
      namingPattern: "{paletteName}/{step}/{scale}",
      generationType: "primitives",
      aliasesToLayer: undefined,
      estimatedVariableCount: 768 // 4 palettes × 24 steps × 8 scales
    },
    {
      id: "primitives-functional",
      order: 0,
      collectionName: "00_Primitives_Functional",
      displayName: "Primitives Functional",
      description: "Functional palettes: positive, negative, warning, informative",
      enabled: true,
      isAutoGenerated: true,
      namingPattern: "{paletteName}/{step}/{scale}",
      generationType: "primitives",
...
export function validateLayerConfig(config: LayerMappingConfig): {
  ...
  // Check if primitive layers exist and are enabled
  const primitiveLayers = config.layers.filter(l => l.generationType === 'primitives');
  if (primitiveLayers.length === 0) {
    errors.push('No primitives layers found - at least one primitives layer is required');
  } else {
    const disabledPrimitives = primitiveLayers.filter(l => !l.enabled);
    if (disabledPrimitives.length > 0) {
      warnings.push(`Primitives layers disabled: ${disabledPrimitives.map(l => l.displayName).join(', ')} - variable generation may fail`);
    }
  }
